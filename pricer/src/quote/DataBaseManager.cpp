...
b_oid DataBaseManager::get_stock_id(const char* name){
  ...
  auto cursor = coll.find(make_document(kvp("name", name)));
...
Spot DataBaseManager::getSpot(const std::string& date, const char* name){
  b_date bdate = read_date(date, 1);
  double price = getSpot(bdate, name);
  Spot *spot = new Spot(date, price);
  return *spot;
}

double DataBaseManager::getSpot(b_date date, const char* name){
...
list<Spot> DataBaseManager::getSpots(const std::string& start_date, const std::string& end_date, const char* name){
  mongocxx::collection coll = db["spots"];
