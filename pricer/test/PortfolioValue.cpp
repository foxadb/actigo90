...
int main(int argc, char** argv){
  ...
  double free_risk_part = 0.0;
  double risked_part = 0.0;

  // mc->rebalanceAtSpecificDate(past,  convertedDate, delta, price);
  PnlVect *current_spots = pnl_vect_create_from_scalar(actigoSize, 0.0);
  ...
  while ( current_date <= input_date){
    ...
    mc->rebalanceAtSpecificDate(past, convertedDate, current_delta, price);
    ...
    if (current_date == 1428451200){
      free_risk_part = price - risked_part;
    }
    else
    {
      free_risk_part = free_risk_part * exp(rEur*(1.0/365.0))
            - risked_part + pnl_vect_scalar_prod(previous_delta, current_spots);
    }
    portfolio_value = free_risk_part + risked_part;
    dbManager->postPrice(current_date, price, portfolio_value);
